...
7. `archived_by` is resolvable. If `--by <email>` is supplied, use it. Otherwise write the literal placeholder `TODO@example.com` into the `archived_by` frontmatter field and append a TODO note to the thread body reminding the user to fix it. **No `git` invocation and no env-var read** — rc4 keeps discard-thread shell-free. Precondition always succeeds; never refuses.
...
| (retired in rc4 — `archived_by` defaults to `TODO@example.com` when `--by` not supplied; user fixes later)            | — | (never refuses)                                     | — | (no-op rownfigure git                                       |
