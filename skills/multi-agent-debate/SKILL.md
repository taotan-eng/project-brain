...
| `personas`            | flag or prompt                  | cond.    | Comma-separated list of persona names drawn from § 10.2, e.g. `--personas=fresh-eyes-reader,red-team,cross-ref-auditor`. If omitted, the skill prompts the user (multi-select) with the project's full § 10.2 roster. |
...
Prompt strategy: resolve `artifact_path` from cwd. Infer `scope` from the path prefix. Infer `mode` from artifact status and round state. For `open`, ask the user to multi-select `personas` from § 10.2 if not supplied; if no § 10.2 personas are configured, require at least one ad-hoc persona. Ask for `feedback` via a follow-up. For `close`, ask the user to pick `patches_status` between three options: "Applied in a separate commit" / "Partially applied — some rejected" / "Rejected — don't land any".
