...
3. **Flip thread frontmatter (commit 1 of 1).** In `thoughts/threads/[thread_slug]/thread.md`:
...
6. **Commit staging.** Stage the thread directory: `git add thoughts/threads/[slug]/`. The index files will be staged in the Final step.
...
- If the rebuild returns exit 0: proceed to commit; stage both index files along with the thread directory in a single commit: `git add thoughts/ && git commit -m "chore([thread_slug]): discard promotion — PR closed unmerged ([pr_url])"`.
...
| `git add thoughts/ && git commit -m …`               | step 7  | Single commit on main; contains frontmatter + regenerated indexes.    |
