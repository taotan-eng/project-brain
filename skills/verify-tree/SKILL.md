...
| V-21  | error    | any artifact    | Artifact filenames and directory names under `project-brain/` must contain no whitespace, no FS-reserved chars (`/\:*?"<>|`), and no control characters. Unicode letters/digits in any script are accepted (e.g. `tree/旅行/行程.md`, `threads/香港转机/`). Reserved-filename exceptions like `NODE.md`, `CONVENTIONS.md`, `README.md`, `thread.md`, `index.md` retain exact casing. (Earlier versions of this rule required ASCII-only; widened in the Unicode-slug commit.) |
| V-22  | error    | artifact        | Artifact's `source_thread` frontmatter matches the parent thread slug on disk (the segment between `threads/` or `archive/` and `artifacts/`), AND resolves to an existing thread in `threads/` or `archive/`. Suppressed when `source_thread` is absent (V-06 handles that). |

### § 11 naming rules

| Code  | Severity | Applies to              | Check                                                                                   |
|-------|----------|-------------------------|-----------------------------------------------------------------------------------------|
| N-01  | warning  | thread, leaf, NODE.md   | `id` frontmatter is a Unicode-friendly kebab-case slug (§ 11.1): 2–40 codepoints, no ASCII uppercase, no whitespace, no FS-reserved chars (`/\:*?"<>|`), no controls, no leading/trailing/doubled hyphens. Unicode letters/digits in any script are accepted (e.g. `auth`, `auth-rotation`, `香港转机`, `データ-モデル`, `café-2`).   |
...
**V-21: filesystem-safe filename constraint.** Artifact filenames and directory names under `project-brain/` must contain no whitespace, no FS-reserved characters (`/\:*?"<>|`), and no control characters. Unicode letters/digits in any script (Latin, CJK, Cyrillic, Arabic, accented Latin, etc.) are accepted — e.g. `tree/旅行/行程.md`, `threads/香港转机/`. Exceptions: special fixed names like `NODE.md`, `CONVENTIONS.md`, `README.md`, `thread.md`, `index.md` — keep their exact casing. Detection: during walk (Process step 1), each filename and directory name (excluding the exceptions) is checked against `is_valid_path_component()` in `verify_tree/model.py`. Cost: O(1) per filename. Error template: `V-21: path component '<p>' contains unsafe characters (whitespace, FS-reserved /\\:*?"<>|, or control chars).` Earlier versions of this rule were ASCII-only; widened in line with CONVENTIONS § 11.1.
