...
- `domain_taxonomy`: **empty by default — no domain folders are pre-created.** The tree starts flat (`tree/.gitkeep` only). Domains emerge organically when `promote-thread-to-tree` lands the first leaf into one — the user names the domain at promote-time, not at init. This avoids saddling fresh installs with an arbitrary placeholder folder that may not match the user's actual structure. (Specifically: the LLM must NOT invent a default like `engineering/` or `product/` — those are NOT shipped defaults; the user names the folder when they promote their first leaf.)
