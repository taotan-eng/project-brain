...
assigned_to: []                # optional; free-form list of owner/collaborator handles (GitHub, Slack, email, whatever the project prefers). Semantics intentionally open per project policy: single-owner, multi-collaborator, or role-based are all valid. Pack skills read and surface this field (e.g. list-threads filters by it) but do NOT enforce any model — teams wire enforcement via CODEOWNERS, branch protection, or their own tooling.
...
| V-21  | any artifact         | Filenames and directory names under `project-brain/` contain no whitespace, no FS-reserved chars (`/\:*?"<>|`), no control characters. Unicode letters/digits in any script are accepted (Latin, CJK, Cyrillic, etc.). Reserved fixed names like `NODE.md`, `CONVENTIONS.md`, `README.md`, `thread.md`, `index.md` retain exact casing. (Was ASCII-only in pre-Unicode-slug versions; widened in line with § 11.1.) |
...
The pack's only constraint is § 11.1 (kebab-case slug). Names are entirely the user's call. The `promote-local.sh` script enforces this with a runtime guardrail: every promotion must pass `--allow-domain=<name>` matching the staged path's top-level. **The flag is the ONLY consent path the script honors.** Thread frontmatter `tree_domain` is documentation (the agent's memory of past blessed choices) but is NOT script-level authority — because the LLM has write access to thread.md, so frontmatter cannot reliably distinguish "user chose this" from "agent fabricated this to satisfy the previous refusal." The flag, by contrast, is visible in the bash tool-call audit, so when the agent fabricates a value the user can spot it in the session transcript. Folder existence on disk is also not consent — folders can be created by prior LLM mistakes, manual mkdir, or half-finished promotions. The rule is universal: every name treated identically, no specific blacklist, no special case for existing folders, no frontmatter bypass. The primary control is the HARD CONSTRAINT block in `skills/promote-thread-to-tree/SKILL.md` requiring agents to ask the user before staging; the script guardrail is the backstop.
