    ...
    def check_v03_soft_links(self, a: Artifact) -> None:
        ...
        for link in links:
            ...
            if ":" in uri and not uri.startswith("/"):
                ...
                projects = self.brain / "projects.yaml"
                if not projects.is_file():
                    continue
                try:
                    data = yaml.safe_load(projects.read_text()) or {}
                except yaml.YAMLError:
                    self.add(
                        "V-03",
                        a,
                        "projects.yaml malformed; cannot resolve alias.",
                    )
                    continue
                ...
                if alias not in aliases:
                    self.add(
                        "V-03",
                        a,
                        f"soft_link alias {alias!r} not found in projects.yaml.",
                    )
                    continue
                target_root = Path(aliases[alias])
                if not target_root.is_absolute():
                    target_root = self.brain / target_root
                target = target_root / sub.lstrip("/")
