...
def rebuild_index(brain: Path, dry_run: bool) -> int:
    ...
    # Deterministic banner: use the high-water mark of source frontmatter
    # timestamps instead of datetime.now(), so repeated rebuilds on an
    # unchanged brain produce byte-identical output. This keeps diff-based
    # CI gates meaningful and removes E2E noise.
    timestamp = _high_water_timestamp(threads, leaves)
...
# Fields we scan across threads + leaves to compute the high-water
# timestamp. These cover every lifecycle event a user can persist via
# frontmatter. Ordering doesn't matter — we pick the max string.
_TIMESTAMP_FIELDS = (
    "last_modified_at",
    "archived_at",
    "parked_at",
...
def _high_water_timestamp(
    threads: list[Artifact],
    leaves: list[Artifact],
) -> str:
    """Return the latest ISO-8601 timestamp across source frontmatter.

    If no timestamps are present (e.g. empty brain on first install), fall
    back to the epoch sentinel so output remains deterministic and obvious.
    """
    candidates: list[str] = []
    for a in threads + leaves:
        for f in _TIMESTAMP_FIELDS:
            v = a.frontmatter.get(f)
            if isinstance(v, str) and v:
                candidates.append(v)
    if not candidates:
        return "1970-01-01T00:00:00Z"
    # ISO-8601 sorts lexicographically when all values share the same
    # timezone suffix. Most fixtures use Z; we tolerate "+00:00" too by
    # preferring the alphabetically-greater string, which matches semantics
    # for values in the same timezone.
...
def _render_thread_index(
    ...
    lines.append(f"Source data as of: {timestamp}")
    lines.append("-->")
    lines.append("")
    lines.append("---")
